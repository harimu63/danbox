...
function index()
	...
	-- JSON/API endpoints (all leaf, no menu entry)
...
local function safe_path(rel)
	...
	local full = base .. "/" .. rel
	local real = fs.realpath(full)
	-- allow non-existent files (new file save) as long as their parent resolves inside base
	if not real then
		real = full
	end
...
-- ---------------------------------------------------------------------
-- app config (Enable / bin path / config dir & file / start delay)
-- ---------------------------------------------------------------------
...
-- ---------------------------------------------------------------------
-- editor / mini file manager (sandboxed to config_dir)
-- ---------------------------------------------------------------------
...
function action_file_list()
	...
	if dir then
		for f in dir do
			if f ~= "." and f ~= ".." then
				local st = fs.stat(cfg.config_dir .. "/" .. f)
				table.insert(list, {
					name = f,
	...
	http.prepare_content("application/json")
	http.write_json({ dir = cfg.config_dir, files = list })
end
...
function action_log_data()
	local out = sys.exec("logread 2>/dev/null | grep -i danbox | tail -n 300") or ""
	http.prepare_content("application/json")
